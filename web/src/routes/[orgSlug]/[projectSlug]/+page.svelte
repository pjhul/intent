<script lang="ts">
	...
	onMount(async () => {
		try {
			await cohorts.loadWithContext(orgSlug, projectSlug);
...
<div class="p-6 max-w-7xl mx-auto">
	...
	<div class="flex items-center justify-between mb-6">
		...
		<a href="/{orgSlug}/{projectSlug}/cohorts/new" class="btn btn-primary">
			...
			{:else}
				<div class="grid gap-4">
					{#each filteredCohorts as cohort (cohort.id)}
						<CohortCard {cohort} baseUrl="/{orgSlug}/{projectSlug}" />
					{/each}
				</div>
			{/if}
		</div>
						...
						{#each $membershipChanges as change}
							<div class="text-sm">
								...
								<div class="ml-4 text-gray-500">
									...
									<a
										href="/{orgSlug}/{projectSlug}/cohorts/{change.cohort_id}"
